...
library(lubridate)
not_cancelled %>% mutate(dep_date_time=ymd_hm(paste(year,month,day," ",dep_time %/% 100,dep_time %% 100))) %>% group_by(origin,dep_date_time) %>% mutate(lag_eff = lag(dep_delay,order_by=dep_time)) %>% arrange(year, month, day, dep_time,dep_date_time,dep_delay,lag_eff)
...
flights %>% group_by(year,month,day) %>%
  filter(!is.na(dep_delay)) %>% 
  mutate(lag_eff = lag(dep_delay,order_by=dep_time)) %>% select(year:day,dep_time,dep_delay,lag_eff) %>%  arrange(year, month, day, desc(dep_time)) %>% ggplot(aes(x=dep_time,y=lag_eff)) + geom_point()
